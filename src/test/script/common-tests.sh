...
(cd "$test_dir" && find . -name '*.deca' | egrep -v '^[0-9ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_/.+-]*$' 2>/dev/null) && die "caracteres illicites dans les noms des tests ou des repertoires"
